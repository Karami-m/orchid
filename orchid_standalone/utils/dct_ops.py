...
"""
The code is based on torch implementation of DCT in 
https://github.com/zh217/torch-dct
"""

def dct1_rfft_impl(x, dim=-1):
    return torch.view_as_real(torch.fft.rfft(x, dim=dim))


def dct_fft_impl(v, dim=-1):
    return torch.view_as_real(torch.fft.fft(v, dim=dim))


def idct_irfft_impl(V, dim=-1):
    return torch.fft.irfft(V, n=V.shape[dim], dim=dim)
...
def idct(X, norm=None, dim=-1):
    ...
    v = idct_irfft_impl(torch.complex(V_r, V_i))
